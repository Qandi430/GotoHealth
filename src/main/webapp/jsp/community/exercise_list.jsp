				...
				<c:forEach var="board" items="${list}" >
					<c:if test="${board.type == '운동방법'}">
						<tr class="title">
							...
							<td><a href="detail.do?no=${board.no}&typeParam=exercise" style="color: black; text-decoration: none;">${board.title}</a></td>
							<td>${board.writer}</td>
				...
				<ul class="paging cf">
						...
						<c:when test="${startPage > 1}">
							<c:choose>
								<c:when test="${empty word and empty search}">
									<li style="border-left: 1px solid #e6e0e7; border-radius: 3px 0 0 3px; background-image: url('/gth/img/community/arrow_double_left_paging.png');"><a href="exerciseList.do?pageNum=1"></a></li>
								</c:when>
								<c:otherwise>
					...
					<c:choose>
						<c:when test="${startPage > pageBlock}">
							<c:choose>
								<c:when test="${empty word and empty search}">
									<li style="background-image: url('/gth/img/community/arrow_left_paging.png');"><a href="exerciseList.do?pageNum=${startPage - 5}"></a></li>
								</c:when>
								<c:otherwise>
					...
					<c:forEach var="i" begin="${startPage}" end="${endPage}">
						<c:choose>
							...
							<c:otherwise>
								<c:choose>
									<c:when test="${empty word and empty search}">
										<li><a href="exerciseList.do?pageNum=${i}">${i}</a></li>
									</c:when>
									<c:otherwise>
										<li><a href="exerciseList.do?pageNum=${i}&word=${word}&search=${search}">${i}</a></li>
									</c:otherwise>
								</c:choose>
							</c:otherwise>		
						</c:choose>				
					...
					<c:choose>
						<c:when test="${endPage < pageCount}">
							<c:choose>
								<c:when test="${empty word and empty search}">
									<li style="background-image: url('/gth/img/community/arrow_right_paging.png');"><a href="exerciseList.do?pageNum=${startPage + 5}"></a></li>
								</c:when>
								<c:otherwise>
									<li style="background-image: url('/gth/img/community/arrow_right_paging.png');"><a href="exerciseList.do?pageNum=${startPage + 5}&word=${word}&search=${search}"></a></li>
								</c:otherwise>
							</c:choose>
						</c:when>
						<c:otherwise>
					...
					<c:choose>
						<c:when test="${endPage < pageCount}">
							<c:choose>
								<c:when test="${empty word and empty search}">
									<li style="border-radius: 0 3px 3px 0; background-image: url('/gth/img/community/arrow_double_right_paging.png');"><a href="exerciseList.do?pageNum=${pageCount}"></a></li>
								</c:when>
								<c:otherwise>
									<li style="border-radius: 0 3px 3px 0; background-image: url('/gth/img/community/arrow_double_right_paging.png');"><a href="exerciseList.do?pageNum=${pageCount}&word=${word}&search=${search}"></a></li>
								</c:otherwise>
							</c:choose>
						</c:when>
						<c:otherwise>
