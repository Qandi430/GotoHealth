				...
				<c:forEach var="board" items="${list}" >
					<c:if test="${board.type == '운동방법'}">
						<tr class="title">
							...
							<td><a href="detail.do?no=${board.no}&typeParam=exercise&pageNum=${currentPage}" style="color: black; text-decoration: none;" onclick="return doAction(${user.id})">${board.title} <c:if test="${board.commentCnt != 0}"> [${board.commentCnt}]</c:if></a></td>
							<td>${board.writer}</td>
