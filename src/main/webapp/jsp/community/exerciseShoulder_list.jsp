				...
				<c:forEach var="board" items="${list}" >
					<c:if test="${board.type == '운동방법' && board.category == '어깨'}">
						<tr class="title">
							...
							<td><a href="detail.do?no=${board.no}&typeParam=exerciseShoulder&pageNum=${currentPage}" style="color: black; text-decoration: none;" onclick="return doAction(${user.id})">${board.title} <c:if test="${board.commentCnt != 0}"> [${board.commentCnt}]</c:if></a></td>
							<td>${board.writer}</td>
				...
				<ul class="paging cf">
						...
						<c:when test="${startPage > 1}">
							<c:choose>
								<c:when test="${empty word and empty search}">
									<li style="border-left: 1px solid #e6e0e7; border-radius: 3px 0 0 3px; background-image: url('/gth/img/community/arrow_double_left_paging.png');"><a href="shoulderList.do?pageNum=1&arrays=${arrays}"></a></li>
								</c:when>
								<c:otherwise>
									<li style="border-left: 1px solid #e6e0e7; border-radius: 3px 0 0 3px; background-image: url('/gth/img/community/arrow_double_left_paging.png');"><a href="shoulderList.do?pageNum=1&word=${word}&search=${search}&arrays=${arrays}"></a></li>
								</c:otherwise>
							</c:choose>
						</c:when>
						<c:otherwise>
					...
					<c:choose>
						<c:when test="${startPage > pageBlock}">
							<c:choose>
								<c:when test="${empty word and empty search}">
									<li style="background-image: url('/gth/img/community/arrow_left_paging.png');"><a href="shoulderList.do?pageNum=${startPage - 5}&arrays=${arrays}"></a></li>
								</c:when>
								<c:otherwise>
									<li style="background-image: url('/gth/img/community/arrow_left_paging.png');"><a href="shoulderList.do?pageNum=${startPage - 5}&word=${word}&search=${search}&arrays=${arrays}"></a></li>
								</c:otherwise>
							</c:choose>
						</c:when>
						<c:otherwise>
					...
					<c:forEach var="i" begin="${startPage}" end="${endPage}">
						<c:choose>
							...
							<c:otherwise>
								<c:choose>
									<c:when test="${empty word and empty search}">
										<li><a href="shoulderList.do?pageNum=${i}&arrays=${arrays}">${i}</a></li>
									</c:when>
									<c:otherwise>
										<li><a href="shoulderList.do?pageNum=${i}&word=${word}&search=${search}&arrays=${arrays}">${i}</a></li>
									</c:otherwise>
								</c:choose>
							</c:otherwise>		
						</c:choose>				
					...
					<c:choose>
						<c:when test="${endPage < pageCount}">
							<c:choose>
								<c:when test="${empty word and empty search}">
									<li style="background-image: url('/gth/img/community/arrow_right_paging.png');"><a href="shoulderList.do?pageNum=${startPage + 5}&arrays=${arrays}"></a></li>
								</c:when>
								<c:otherwise>
									<li style="background-image: url('/gth/img/community/arrow_right_paging.png');"><a href="shoulderList.do?pageNum=${startPage + 5}&word=${word}&search=${search}&arrays=${arrays}"></a></li>
								</c:otherwise>
							</c:choose>
						</c:when>
						<c:otherwise>
					...
					<c:choose>
						<c:when test="${endPage < pageCount}">
							<c:choose>
								<c:when test="${empty word and empty search}">
									<li style="border-radius: 0 3px 3px 0; background-image: url('/gth/img/community/arrow_double_right_paging.png');"><a href="shoulderList.do?pageNum=${pageCount}&arrays=${arrays}"></a></li>
								</c:when>
								<c:otherwise>
									<li style="border-radius: 0 3px 3px 0; background-image: url('/gth/img/community/arrow_double_right_paging.png');"><a href="shoulderList.do?pageNum=${pageCount}&word=${word}&search=${search}&arrays=${arrays}"></a></li>
								</c:otherwise>
							</c:choose>
						</c:when>
						<c:otherwise>
