		...
		<div class="container page_faq">
			<div class="inner">
				...
				<c:forEach var="board" items="${list}" >
					<c:if test="${board.type == '운동방법' && board.category == '등'}">
						<tr class="title">
							<td>${board.no}</td>
							<td>${board.category}</td>
							<td><a href="detail.do?no=${board.no}&typeParam=exerciseBack&pageNum=${currentPage}" style="color: black; text-decoration: none;" onclick="return doAction(${user.id})">${board.title} <c:if test="${board.commentCnt != 0}"> [${board.commentCnt}]</c:if></a></td>
							<td>${board.writer}</td>
